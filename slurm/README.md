...
The 2026-08-05 run, on `ht1`, was exactly:

```bash
srun --mem=256G --cpus-per-task=8 --time=02:00:00 \
  uv run pubmed2db --data-dir data/ export --format json --out data/json --shards 16
# 12:14:50 started, 12:32:56 finished
```

Treat **256 GB** as the working memory figure until a new one is measured — it
was stable across all three runs, and is why this needs a big node; do not copy
the loader's 64 GB. Time is the cheap dimension: two hours is generous margin.
...
- **CPUs now help the writing too.** DuckDB parallelizes the snapshot, the
  ...
  idling behind a single Python thread. Note those runs did *not* pass
  `--threads`, so DuckDB sized its own pool from the node's cores.
